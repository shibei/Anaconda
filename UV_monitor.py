...
def getRawData(day, cycle):
    ...
    uvBase['comm']['apData'] = "SELECT * FROM `ap_count` WHERE     `log_date` = '%s' OR `log_date` = '%s' ORDER BY     `log_date` DESC" % (day2, day1)

    uvBase['comm']['uvData'] = "SELECT * FROM `funnel_model2` WHERE     `log_date` = '%s' OR `log_date` = '%s' ORDER BY     `log_date` DESC" % (day2, day1)

    devBase['comm']['devInfo'] = "SELECT %s FROM `dev_info`     WHERE `date` = '%s' ORDER BY `clock` DESC LIMIT 0, 1000" % (devInfoFields, day0)
